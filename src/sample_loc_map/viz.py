...
def make_marker(ser: pd.Series, x='Lon', y='Lat', prop_columns=['icon', 'text_color', 'background_color'], parent=None, **kwargs):
    """
    ...
    **kwargs : dict
    ...
    props = dict(inner_icon_style="padding-top:0px; font-size:14px", border_width=1, iconSize=(25, 25))
    ...
    popup_html = popup.to_html(col_space=80, border=0, justify='left', classes="table table-hover table-responsive")
...
def add_tiles(map):
    ...
    tiles[key] += folium.TileLayer('Cartodb dark_matter', name="Cartodb-dark", **props),
    tiles[key] += folium.TileLayer(**GOOGLE_SATELLITE, name="Google Earth", **props),
    tiles[key] += folium.TileLayer(**GOOGLE_TERRAIN, name="Google Terrain", control=False, max_zoom=14),
    ...
    folium.plugins.GroupedLayerControl(tiles, exclusive_groups=True, collapsed=False).add_to(map) 
